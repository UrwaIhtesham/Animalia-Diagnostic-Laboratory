...
const OurTests = () => {
  ...
  const fetchTests = async () => {
    // try {
    //   const response = await axios.get("http://localhost:5000/api/tests"); // Fixed endpoint to fetch tests
  ...
  const handleAddTests = async (e) => {
    e.preventDefault();
    // try {
    //   await axios.post("http://localhost:5000/api/tests", newTests); // Fixed endpoint to add tests
    //   fetchTests();
    //   setNewTests({
    //     name: "",
    //     testfees: "",
  ...
  return (
    <Box m="20px">
      ...
      >
        <DataGrid rows={tests} columns={columns} getRowId={(row) => row.testId} />
      </Box>
